...
def enlarge_dataset(dataframe : DataFrame,
                    size=None, feature_cols=None,
                    label_col=None, **kwargs):
    ...
    if n <= size:
        ...
        columns = [
            F.col(i) if i not in feature_cols
            else F.col(i)+F.rand() for i in extra_df.columns
        ]
...
def create_nan_labels(sc, dataframe, label_col, fraction=None, **kwargs):
    ...
    dict_missing_labels = _compute_fraction(
        sc=sc, dataframe=dataframe, label_col=label_col,
        fraction=fraction, **kwargs
    )
    # print(dict_missing_labels)
    # create the new schema
    schema = dataframe.schema.add(
        'id', data_type=T.IntegerType(), nullable=False)

    # add id to dataframe
    rdd = dataframe.rdd.zipWithIndex().map(lambda x: (*x[0], x[1]))
    new_data_frame = spark.createDataFrame(rdd, schema=schema)
    labeled_id = (new_data_frame
        .sampleBy(col=label_col,
                  fractions=dict_missing_labels)
        .select('id')
        .collect()
    )
    bc_labels = sc.broadcast(
    ...
    return new_data_frame.withColumn(
        colName='missing_'+label_col, col=correct_label
    )
...
def _compute_fraction(sc, dataframe, label_col, fraction=None, **kwargs):
    if fraction:  # set all labels to same fraction
        ...
        dict_missing_labels = (dataframe
            ...
            .map(lambda x: (x['label'], bcast.value))
            .collectAsMap()
        )
    else:
...
def run_experiment(sc, dataframe, label_col,
                   feature_cols=None, data_size=None,
                   fractions=None, **kwargs):
    ...
    ex = Experiments.Experiments(n_trials=5, data_size=1000)
