...
def label_propagation(sc, data_frame=None, id_col='id', label_col='label', feature_cols=None, **kwargs):
    ...
    row_summed_matrix = demon_matrix.entries.flatMap(lp_helper.triangle_mat_summation)\
    ...
    col_summed_matrix = (transition_rdd.flatMap(lp_helper.triangle_mat_summation)
        .reduceByKey(lambda x, y: x + y).collectAsMap())
