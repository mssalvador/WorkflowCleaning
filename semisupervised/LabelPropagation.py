...
def create_complete_graph(
    ...
    def _compute_squared_distance(vec_x, vec_y):
        return float(np.linalg.norm(vec_x-vec_y, ord=2)**2)
    ...
    compute_distance_squared = F.udf(
        lambda x, y: math.exp(- _compute_squared_distance(x, y) / (sigma ** 2)),
        T.DoubleType()
    )
    return (df_cleaned
        .select(*a_names)
        .join(df_cleaned.select(*b_names))
        .withColumn('weights_ab',
...
def _delta_func(context, labels, **kwargs):

    difference = {}

    label_col = kwargs.get('label_column', 'initial_label')
    for key, val in labels.items():
        difference[key] = list(zip(range(context.constants['n'].value), val, np.abs(
            context.constants[label_col].value[key] - val)
                           <= context.constants['tol'].value))
    return difference
...
def label_propagation(
    ...
    df_transition_values = df_with_weights.select(
        F.col('a_'+id_col).alias('row'),
        F.col('b_'+id_col).alias('column'),
        F.col('weights_ab'),
        F.col('a_'+label_col).alias('label')).cache()
    df_transition_values.take(1)

    generate_summed_weights(
        context= label_context_set,
        weights= df_transition_values
    )
    ...
    udf_generate_intial_label = F.udf(lambda x: _label_by_row(label_context.constants['k'].value, x), T.ArrayType(T.DoubleType()))
    ...
    udf_clamp_labels = F.udf(lambda row:  label_context.constants['clamping_label'].value , T.BooleanType())
    ...
    df_transition_matrix = (df_normalized_transition_values
                            ...
                            .withColumn(
        colName= 'row_trans', col= udf_sorts('row_trans'))
                            .withColumn(
        colName= 'label', col= F.when(F.col('label') == None, value=np.NaN).otherwise(F.col('label')))
                            .withColumn(
        colName= 'initial_label', col= udf_generate_intial_label('label'))
                            .withColumn(
        colName= 'is_clamped', col= ~F.isnan('label'))
                            .withColumn(
        colName= 'summed_transition', col= udf_summation(F.col('row_trans')) )
                            .withColumn(
        colName= 'row_trans', col= udf_normalization('row_trans', 'summed_transition'))
    ...
    dict_initial_label = generate_label(
        ...
        data_frame= df_transition_matrix,
        label_weights= 'initial_label'
    )
    ...
    udf_arg_max = F.udf(lambda x: float(np.argmax(np.array(x))), T.DoubleType())
    return  df_transition_matrix.withColumn(colName='label', col= udf_arg_max(F.col('initial_label')))